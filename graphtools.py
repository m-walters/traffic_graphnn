...
class graphplot:
    ...
    def __init__(self, nodes_obj, idxlist=[], figsize=None, figure=None, axis=None, usekm=False):
        ...
        if usekm:
            if self.noderadius < 0.1:
                self.noderadius = 0.1
            if self.noderadius > 0.5:
                self.noderadius = 0.5
        else:
            if self.noderadius < 0.001:
                self.noderadius = 0.001
            if self.noderadius > 0.01:
                self.noderadius = 0.01
...
class viewer(graphplot):
    ...
    def __init__(self, nodes_obj, window=None, figsize=None, idxlist=[], usekm=False):
        ...
        super().__init__(
            nodedict, idxlist, (self.figsize[0] * 0.75, self.figsize[1] * 0.75),
        ...
        self.window = Rectangle(
            (xmin, ymin), xmax - xmin, ymax - ymin, fill=False, linewidth=1.6, edgecolor="orangered"
        )
    ...
    def on_press(self, event):
        ...
        if event.inaxes != self.window.axes:
            return
